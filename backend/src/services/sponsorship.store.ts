...
const VITE_PAY_API_URL = process.env.PAY_API_URL || null;
...
export async function topupSponsorCredits(
    ...
    try {
        ...
        // integrate Stablecoin EFT deposit
        const payloadFundUser = {
            sponsorId: sponsorId,
            amount_cents: amt,
            currency: 'ZAR',
            description: `EFT deposit ${amt} approved for ${sponsorId}`,
            idempotency_key: idempotency_key
        }
        const fundResult = await fetch(`${VITE_PAY_API_URL}/fund-user`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify(payloadFundUser)
        })
        console.log('Just for logs :)')
    ...
    try {
        await dynamo.updateItem({
            ...
            ExpressionAttributeValues: {
                ':z': 0,
                ':d': Number(delta_allocated_cents || 0),
                ':u': now,
                ':sid': sponsorId,
                ':stid': studentId
            }
...
export async function listSponsorStudentAggregates(studentId: string): Promise<Array<{
    sponsorId: string;
...
export async function updateSponsorAggregate(sponsorId: string, deltas: {
...
export async function incrementStudentSponsorCategoryUsed(
    ...
    try {
        await dynamo.updateItem({
            ...
            ExpressionAttributeValues: {
                ':z': 0,
                ':uinc': Number(delta_used_cents || 0),
                ':u': now,
                ':sid': sponsorId,
                ':stid': studentId,
                ':cat': category
        ...
        console.error('Failed to increment used_total_cents for StudentSponsorCategoryBudget', {
            sponsorId,
            studentId,
            category,
            err
...
export async function listStudentSponsorCategoryBudgets(studentId: string): Promise<Array<{
    sponsorId: string;
    category: SpendCategory;
    allocated_total_cents: number;
    used_total_cents: number;
    updated_at?: string
}>> {
    try {
        const resp = await dynamo.query({
...
export async function allocateBudgets(
    ...
    updated: Array<{ category: SpendCategory; allocated_total: number; used_total: number; available: number }>
}> {
    ...
    for (const a of allocations) {
        ...
        try {
            ...
            console.error('Failed to persist AllocationLot to DynamoDB', {
                sponsorId,
                studentId,
                category: a.category,
                err
        ...
        try {
            ...
            console.error('Failed to persist allocation ledger to DynamoDB', {
                sponsorId,
                studentId,
                category: a.category,
                err
        ...
        try {
            ...
            console.error('Failed to update StudentSponsorCategoryBudget', {
                sponsorId,
                studentId,
                category: a.category,
                err
    ...
    // transfer from Sponsor to Student
    const payloadFundUser = {
        sponsorId: sponsorId,
        studentId: studentId,
        amount_cents: totalRequested,
        currency: 'ZAR',
        description: `Sponsor amount ${totalRequested} approved from ${sponsorId} to ${studentId}`,
        idempotency_key: idempotency_key
    }
    console.log("fund student");
    console.log(payloadFundUser);
    const fundResult = await fetch(`${VITE_PAY_API_URL}/sponsor-user`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
    ...
    const resFundResult = await fundResult.json();
    console.log('Just for logs :)')
    console.log({resFundResult})
    return response;
}
    ...
    try {
        ...
        for (const cat of cats) {
            ...
            result.push({
                ...
                allocated_total: vals.allocated,
                used_total: vals.used,
                available
            });
        }
        return result;
...
export async function listStudentTransactionsFromDB(studentId: string, params?: {
    Limit?: number;
    ExclusiveStartKey?: any
}): Promise<{ Items: DynamoItem[]; LastEvaluatedKey?: any }> {
...
export async function prepareTransaction(
    ...
    /// check on Stablecoin if amount is available
    const payload = {
        from_user_id: studentId,
        to_user_id: params.merchantId,
        amount_cents: covered,
    }
    const checkResult = await fetch(`${VITE_PAY_API_URL}/can-pay`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json',
        },
...
export async function confirmTransaction(
    ...
    try {
        const studentCanPay = {
            studentId: studentId,
            amount_cents: Number(tx.amount_covered_cents || 0),
            currency: 'ZAR',
            description: `Payment ${Number(tx.amount_covered_cents || 0)} from {studentId} to ${tx.merchantId}`,
            idempotency_key: idempotency_key
        }
        const canPayResult = await fetch(`${VITE_PAY_API_URL}/can-pay`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify(studentCanPay)
        })
        console.log('Just for logs :) Student canPayResult')
        console.log({canPayResult})
        const resCanPayResult : any = await canPayResult.json();
        console.log('Just for logs :)')
        console.log({resCanPayResult})
        if (resCanPayResult
            && resCanPayResult.hasOwnProperty('result')
            && !resCanPayResult['result']){
            throw new Error('Insufficient Funds.');
        }

        await dynamo.transactWrite({TransactItems: transactItems});

        // // transfer from Student wallet to merchant
        const studentPayMerchant = {
            studentId: studentId,
            merchantId: tx.merchantId,
            amount_cents: Number(tx.amount_covered_cents || 0),
            currency: 'ZAR',
        ...
        const payResult = await fetch(`${VITE_PAY_API_URL}/pay-user`, {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json',
            },
            body: JSON.stringify(studentPayMerchant)
        })
        console.log('Just for logs :) Student to Merchant')
...
export async function reverseAllocations(
    ...
    for (const r of normalized) {
        ...
        try {
            ...
            console.error('reverseAllocations: failed to persist reversal ledger', {
                sponsorId,
                studentId,
                category: r.category,
                err
    ...
    try {
        ...
        if (lookup) {
            ...
            const item = await dynamo.getItem({
                Pk: `SPONSOR#${sponsorId}` as any,
                Sk: `EFT_NOTIFY#${created_at}#${eftId}`
            });
...
export async function listEFTDepositNotifications(
    ...
    try {
        ...
        if (limit || cursor) {
            ...
            return {
                items: items.map(it => mapEFTItem(it, sponsorId)),
                total: items.length,
...
export async function listAllEFTDepositsAsync(params?: {
    status?: EFTStatus | 'all';
    page?: number;
    page_size?: number;
    limit?: number;
    cursor?: any
...
export async function approveEFTDeposit(
            ...
            TransactItems: [
                {
                    Update: {
                        ...
                        ExpressionAttributeValues: {
                            ...
                            ':u': now,
                            ':expected': 'new',
                            ':gpk': `SPONSOR#${eft.sponsorId}`,
                            ':gsk': `EFT#allocated#${eft.created_at}`
                        }
                    }
                },
    ...
    const resp = await topupSponsorCredits(eft.sponsorId, approved, idempotency_key, {
